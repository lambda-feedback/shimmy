...
func TestMuEdV010_EncodeHealth_MatchesFreeFunction(t *testing.T) {
	...
	for _, passed := range []bool{true, false} {
		...
		assert.Equal(t, runtime.MuEdToHealthResponse(result, false), a.EncodeHealth(result, false))
	}
}
...
func TestMuEdV010_Chat_MatchesFreeFunctions(t *testing.T) {
	...
	chatReq := runtime.MuEdChatRequest{Messages: []runtime.MuEdChatMessage{{Role: "USER", Content: "hi"}}}
	...
	assert.Equal(t, runtime.MuEdToChatHealthResponse(healthResult, false), a.EncodeChatHealth(healthResult, false))
}
